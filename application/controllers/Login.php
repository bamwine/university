...
class Login extends CI_Controller {
	...
	$ff=	check_lice();
		if($ff==1) redirect(base_url() . 'index.php?login/key_register', 'refresh');else{
    ...
    {
        $this->load->view('backend/vaule_k');
    }
	 ...
	 if ($sys_cod==$sys_cod_chk) 
        {
			...
			$this->db->where('systemco' , $sys_cod);
		 $this->db->update('valid' , array('tkl' => $key,'startd' => $today,'enda' => $expire));	
		redirect(base_url() . 'index.php?login/', 'refresh');
		
		}
	
	else{redirect(base_url() . 'index.php?login/key_register', 'refresh');}
...
function check_lice()
    {	
	 ...
	 $date_now = date("Y-m-d ");
		...
		$queryr = $CI->db->query('SELECT * FROM valid');
		$tkl = $queryr->row()->tkl;
