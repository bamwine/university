...
class Teacher extends CI_Controller
{
	...
	function student_information($class_id = '')
	{
		...
		$page_data['page_title'] 	= get_phrase('student_information'). " - ".get_phrase('Courses')." : ".
	...
	function student_marksheet($student_id = '') {
        ...
        $page_data['page_title'] =   get_phrase('marksheet_for') . ' ' . $student_name . ' (' . get_phrase('Courses') . ' ' . $class_name . ')';
    ...
    function marks($exam_id = '', $class_id = '', $subject_id = '')
    {
	 ...
	 if ($this->input->post('operation') == 'update') {
			...
			$query = $this->db->get_where('mark' , array('exam_id' =>$this->input->post('exam_id') , 'class_id' =>$this->input->post('class_id'), 'subject_id' =>$this->input->post('subject_id')));
            $query_le = $this->db->get_where('class' , array( 'class_id' =>$class_id));
			$levels=$query_le->row()->class_level;
            ...
            foreach($students as $row) {
                ...
                $agg_hold=$this->get_aggretes_makr($data['mark_obtained'],$levels);
				...
				$this->db->update('mark', array('subj_name' => $data['subj_name'],'aggr' => $agg_hold ,'mark_obtained' => $data['mark_obtained'] , 'comment' => $data['comment'] , 'exam_id' =>$this->input->post('exam_id') , 'class_id' =>$this->input->post('class_id'), 'subject_id' =>$this->input->post('subject_id')));
				$this->save_average_marks($data['student_id'] ,$this->input->post('exam_id'));
				derive_point_grades($data['student_id'] ,$this->input->post('exam_id'));
         ...
         //redirect(base_url() . 'index.php?teacher/marks/' ,'refresh');
            ...
            foreach($students as $row) {
                ...
                $agg_hold=$this->get_aggretes_makr($data['mark_obtained'],$levels);
                //$this->db->where('mark_id', $this->input->post('mark_id_' . $row['student_id']));
                $this->db->insert('mark', array('subj_name' => $data['subj_name'],'aggr' => $agg_hold,'student_id' => $data['student_id'],'mark_obtained' => $data['mark_obtained'] , 'comment' => $data['comment'] , 'exam_id' =>$this->input->post('exam_id') , 'class_id' =>$this->input->post('class_id'), 'subject_id' =>$this->input->post('subject_id')));
				$this->save_average_marks($data['student_id'] ,$this->input->post('exam_id'));
