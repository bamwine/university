...
<?php echo form_open(base_url() . 'index.php?admin/marks' , array('class' => 'form-horizontal form-groups-bordered validate', 'enctype' => 'multipart/form-data', 'method' => 'post'));?>
...
<?php 
         $yos =  $this->crud_model->get_courseunit_term($subject_id,'yos');
		$tos = $this->crud_model->get_courseunit_term($subject_id,'tos');
        $acadmic_period = $this->db->get_where('settings' , array('type' =>'years'))->row()->description;        		
	    $query = $this->db->get_where('mark' , array('exam_id' =>$exam_id , 'class_id' =>$class_id, 'subject_id' =>$subject_id,'acadmic_period' =>$acadmic_period,'tos'=>$tos,'yos'=>$yos));
           
		   if ($query->num_rows() > 0){
			 // $students   =    $this->db->get_where('mark' , array('exam_id' =>$exam_id , 'class_id' =>$class_id, 'subject_id' =>$subject_id,'acadmic_period' =>$acadmic_period))->result_array();	
