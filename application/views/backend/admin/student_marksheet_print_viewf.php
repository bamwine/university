...
<script type="text/javascript">
    ...
    function Popup(data)
    {
        var mywindow = window.open('', 'Final report', 'height=400,width=600');
        mywindow.document.write('<html><head><title>Final report</title>');
        ...
        mywindow.document.write('<link rel="stylesheet" href="assets/css/bamwine.css" type="text/css" />');
...
<div class="container light-shadow"   >
	...
	<?php $queryr = $this->db->query('SELECT class.class_level FROM class INNER JOIN student ON student.class_id = class.class_id WHERE student.student_id = "'.$student_id.'"');
	$class_level = $queryr->row()->class_level;
?>
...
<p class="text-uppercase theme-font broadway" style="font-size:50px;">
   <?php echo $this->db->get_where('settings' , array('type' =>'school_name'))->row()->description;?></p>
            ...
            <p style="font-size:16px;">
				...
				Center No: <?php echo $this->db->get_where('settings' , array('type' =>'school_no'))->row()->description;?><br/> 
			</p>
...
<b><?php get_subject_mark2f($student_id,$row['exam_id'],$subject['subject_id']);?></b>

<b><?php //average_marksf($student_id,$row['exam_id'],$subject['subject_id']);?></b>
<?php
}
?>
...
foreach($examsd as $row){
?>
<td><b><?php //average_marksf($student_id,$row['exam_id']);?></b></td>
<b><?php get_aggreates_comment($student_id,$row['exam_id'],$subject['subject_id']);?></b>
<?php
}
?>
...
<td><i>K</i></td>
 ...
 <?php endforeach;?>
              ...
              <?php if($class_level=='o'){ ?>  AGGREGATE :  <input style="border-bottom:1px dotted #2182C0; width:300px;" class="no-border-top no-border-left no-border-right" value="&nbsp <?php get_aggreates($student_id,$exam_id);?>"/>
				&nbsp;&nbsp;  DIVISION: <input style="border-bottom:1px dotted #2182C0; width:200px;" class="no-border-top no-border-left no-border-right" value="&nbsp <?php get_division($student_id,$exam_id);?>"/>
                    <br/><br/>
					...
					&nbsp&nbspAVERAGE:  <input style="border-bottom:1px dotted #2182C0; width:300px;" class="no-border-top no-border-left no-border-right" value="<?php average_marks($student_id,$exam_id);?>"/>
                    <br/>
			  <?php }?>
